...
pub struct StateLessButton<Message> {
    pressed: bool,
    label: String,
    class: Class,
    on_press: Message,
...
impl<Message> std::fmt::Debug for StateLessButton<Message>
where
...
impl<Message> StateLessButton<Message> {
    ...
    pub fn new(pressed: bool, label: &str, on_press: Message, on_release: Message) -> Self {
        StateLessButton {
            pressed,
...
impl<Message, Renderer> Widget<Message, Renderer> for StateLessButton<Message>
where
...
impl<'a, Message, Renderer> From<StateLessButton<Message>>
    for Element<'a, Message, Renderer>
where
    Renderer: self::Renderer,
    Message: 'static + Copy + std::fmt::Debug,
{
    fn from(button: StateLessButton<Message>) -> Element<'a, Message, Renderer> {
        Element::new(button)
    }
}
